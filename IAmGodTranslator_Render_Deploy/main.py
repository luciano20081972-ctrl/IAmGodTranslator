    ...
    def start(self, kind: str, title: str, worker: Callable[[Callable[..., dict[str, Any]], dict[str, Any]], dict[str, Any]], metadata: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        def run() -> None:
            try:
                ...
                final_status = str(result_status) if result_status in {"blocked", "failed"} else "complete"
                current.update({"status": final_status, "progress": 100, "stage": final_status, "result": result})
                if isinstance(result, dict):
                    if result.get("message"):
                        current["message"] = result.get("message")
                    if result.get("recommended_action"):
                        current["recommended_action"] = result.get("recommended_action")
