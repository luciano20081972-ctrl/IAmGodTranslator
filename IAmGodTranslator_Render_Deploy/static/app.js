...
function renderAuthState() {
  ...
  els.accountMessage.textContent = user ? `${user.email} Â· ${user.role}${user.email_verified ? " Â· verified" : " Â· email not verified"}` : "Use an account to sync bookmarks, ratings, and reading history.";
...
function renderDetail() {
  ...
  els.backupSummary.textContent = `Original ${c.original_files} Â· Reference ${c.reference_files} Â· AI ${c.translated_chapters} Â· Remaining ${c.remaining_chapters} Â· Last backup ${date(n.last_backup_at)}`;
...
async function openReader(chapter, tab = state.readerTab, options = {}) {
  ...
  els.readerProgress.textContent = `Chapter ${position || 1} of ${sorted.length} Â· ${progressPercent(state.currentNovel?.counts)}% AI translated Â· Mode: ${state.readerTab === "ai" ? "AI Translation" : state.readerTab === "reference" ? "Reference Translation" : "Original Story"}`;
